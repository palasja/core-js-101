...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    const ch = strArr[i];
    if (![...strArr.slice(0, i), ...strArr.slice(i + 1)].includes(ch)) { res = ch; break; }
  }
  return res;
}
...
function getDigitalRoot(num) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const el = str[i];
    res += Number.parseInt(el, 10);
  }
  if (res > 9) {
    let res1 = 0;
    const str1 = res.toString().split('');
    for (let i = 0; i < str1.length; i += 1) {
      const el = str1[i];
      res1 += Number.parseInt(el, 10);
    }
    return res1;
  }
  return res;
...
function isBracketsBalanced(/* str */) {
  // if(str.length === 0 || str.length % 2 !== 0) return true;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes.length; i += 1) {
    ...
    if (first !== pathes[i]) first = '';
  }
  for (let i = 0; i < tmp[0].length; i += 1) {
    const cur = tmp[0][i];
    let r = true;
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    ...
    const winEl = row.find((el) => el !== undefined);
    if (winEl !== undefined) {
      let win = true;
      for (let k = 0; k < position.length; k += 1) {
        if (position[i][k] !== winEl) {
          win = false;
        }
      }
      if (win) return winEl;
  ...
  for (let j = 0; j < position.length; j += 1) {
    if (position[0][j] !== undefined) {
      const winEl = position[0][j];
      let win = true;
      for (let k = 0; k < position.length; k += 1) {
        if (position[k][j] !== winEl) {
          win = false;
        }
      }
      if (win) return winEl;
  ...
  const el = position[1][1];
  if (el === position[0][0] && el === position[2][2]) return el;
  if (el === position[2][0] && el === position[0][2]) return el;
  return res;
}
